...
function MenuInner() {
    ...
    const { format, exportMetaList } = useSettingContext()
    ...
    // Export handlers
    const metaList: ExportMeta[] = exportMetaList
    ...
    const getScrollableContainers = (): (HTMLElement | Window)[] => {
        const elements: (HTMLElement | Window)[] = Array.from(document.querySelectorAll<HTMLElement>('*')).filter(
            el =>
                el.scrollHeight > el.clientHeight
                && window.getComputedStyle(el).overflowY.match(/(auto|scroll)/),
        )
        if (document.scrollingElement) {
            elements.push(document.scrollingElement as HTMLElement)
        }
        if (
    ...
    const getPrimaryContainer = useCallback((): HTMLElement | Window => {
        ...
        if (windowScrollHeight > window.innerHeight) {
            const foundWindowOrRoot = containers.find(
                c =>
                    c === window
                    || (document.scrollingElement && c === document.scrollingElement),
            )
            if (foundWindowOrRoot) return foundWindowOrRoot
        }
        return containers.reduce((maxEl, el) => {
            const elScroll = el === window
                ? (document.documentElement.scrollHeight || document.body.scrollHeight)
                : (el as HTMLElement).scrollHeight
            const elClient = el === window
                ? window.innerHeight
                : (el as HTMLElement).clientHeight
            const maxElScroll = maxEl === window
                ? (document.documentElement.scrollHeight || document.body.scrollHeight)
                : (maxEl as HTMLElement).scrollHeight
            const maxElClient = maxEl === window
                ? window.innerHeight
                : (maxEl as HTMLElement).clientHeight
    ...
    const makeButton = (
        svgOrElement: string | preact.JSX.Element,
        ...
        return (
            <div
                ...
                title={title}
            >
                {typeof svgOrElement === 'string'
                    ? <div dangerouslySetInnerHTML={{ __html: svgOrElement }} />
                    : svgOrElement}
            </div>
        )
    }
    ...
    const scrollToTop = () => {
        ...
        if (
            primary === window
            || primary === document.scrollingElement
        ) {
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            let oldTarget: HTMLElement | Window
            if (
                primaryScrollContainerRef.current !== window
                && primaryScrollContainerRef.current !== document.scrollingElement
            ) {
                oldTarget = primaryScrollContainerRef.current as HTMLElement
            }
            else {
                oldTarget = window
            }
            (oldTarget as HTMLElement | Window).removeEventListener('scroll', handleScroll)
            primaryScrollContainerRef.current = newPrimaryContainer
            currentPrimary = newPrimaryContainer
            let newTarget: HTMLElement | Window
            if (
                currentPrimary !== window
                && currentPrimary !== document.scrollingElement
            ) {
                newTarget = currentPrimary as HTMLElement
            }
            else {
                newTarget = window
            }
            (newTarget as HTMLElement | Window).addEventListener('scroll', handleScroll)
            positionContainer()
            updateVisibility()
        }
        ...
        if (
            ...
            targetElement = currentPrimary as HTMLElement
        }
        else {
            targetElement = window
        }
        (targetElement as HTMLElement | Window).addEventListener('scroll', handleScroll)
        window.addEventListener('resize', handleResize)
        const observer = new MutationObserver(() => {
            const newPrimaryOnMutation = getPrimaryContainer()
            if (newPrimaryOnMutation !== primaryScrollContainerRef.current) {
                (targetElement as HTMLElement | Window).removeEventListener('scroll', handleScroll)
            ...
            if (
                ...
                && !document.documentElement.contains(floatingContainerRef.current)
            ) {
    ...
    const scrollTopBtn = showScrollToTop
        ? makeButton(upSvg, scrollToTop, t('Scroll to Top'), 'scroll-top-btn')
        : null
    const scrollBotBtn = showScrollToBottom
        ? makeButton(downSvg, scrollToBottom, t('Scroll to Bottom'), 'scroll-bot-btn')
        : null
